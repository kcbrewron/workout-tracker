	...
	strength: {
		chest: [
			...
			{ 
				...
				formCues: ['Keep shoulder blades retracted', 'Maintain arch in lower back'],
				commonMistakes: ['Bouncing bar off chest', 'Flared elbows'],
				safetyNotes: ['Always use a spotter', 'Use safety bars when available'],
				progressions: ['Dumbbell press', 'Incline bench press', 'Decline bench press'],
				tags: ['barbell', 'upper_body', 'compound', 'strength']
			},
			{
				...
				formCues: ['Keep wrists straight', 'Don\'t let dumbbells drift too far apart'],
				commonMistakes: ['Using too steep an incline', 'Pressing straight up instead of inward'],
				safetyNotes: ['Don\'t lower dumbbells too far below chest level'],
				progressions: ['Flat dumbbell press', 'Incline dumbbell press', 'Incline barbell press'],
				tags: ['dumbbells', 'upper_body', 'compound', 'incline']
			},
			{ 
				id: 'chest_dips', 
				name: 'Chest Dips', 
				...
				formCues: ['Keep elbows close to body', 'Control the descent'],
				commonMistakes: ['Going too low', 'Not leaning forward enough'],
				safetyNotes: ['Don\'t force range of motion if shoulders are tight'],
				progressions: ['Assisted dips', 'Parallel bar dips', 'Weighted dips'],
				tags: ['bodyweight', 'upper_body', 'compound', 'advanced']
			}
		],
		back: [
...
function generatePersonalizedRoutine(userProfile, preferences = {}) {
	...
	// Combine all phases
	routine.exercises = [...warmupExercises, ...mainExercises, ...cooldownExercises];
...
function generateStrengthRoutine(difficulty, equipment, duration) {
	...
	for (let i = 0; i < exerciseCount; i++) {
		...
		const availableExercises = exerciseDatabase.strength[muscleGroup].filter(
			(ex) =>
...
function generateCardioRoutine(difficulty, equipment, duration) {
	...
	const availableSustained = exerciseDatabase.cardio.sustained.filter(
		(ex) =>
			equipment.includes(ex.equipment) || ex.equipment === 'bodyweight' || ex.equipment === 'none'
	);

	const availableInterval = exerciseDatabase.cardio.interval.filter(
		(ex) =>
			(equipment.includes(ex.equipment) || ex.equipment === 'bodyweight') &&
			isDifficultyAppropriate(ex.difficulty, difficulty)
	);
	...
	if (availableSustained.length > 0) {
		const sustainedExercise =
			availableSustained[Math.floor(Math.random() * availableSustained.length)];
	...
	return {
		...
		notes:
			exercise.id === 'running'
				...
				: exercise.id === 'cycling'
					? 'Keep consistent pedaling rhythm'
...
function generateMixedRoutine(difficulty, equipment, duration) {
	const strengthExercises = generateStrengthRoutine(difficulty, equipment, '30_45_mins').slice(
		0,
		3
	);
...
function generateExerciseNotes(exerciseId, difficulty) {
	const exerciseNotes = {
		...
		pull_ups:
			difficulty === 'beginner' ? 'Use assistance band if needed' : 'Focus on controlled movements',
